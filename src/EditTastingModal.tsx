...
export function EditTastingModal({ tasting, userId, onSaved, onClose }: { tasting: EditableTasting | null; userId: string; onSaved: () => void; onClose: () => void }) {
  ...
  return <Modal visible={Boolean(tasting)} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}><SafeAreaView style={styles.page}>
    <View style={styles.header}><Pressable style={styles.close} onPress={onClose}><Ionicons name="close" size={22} color={colors.ink} /></Pressable><Text style={styles.title}>Edit tasting</Text><View style={{ width: 38 }} /></View>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <Text style={styles.label}>RATING</Text><Text style={styles.ratingNumber}>{rating.toFixed(1)}</Text>
      <View style={styles.stars}>{[1, 2, 3, 4, 5].map((star) => <Pressable key={star} onPress={(event) => setRating(star - (event.nativeEvent.locationX < 17 ? 0.5 : 0))}><Ionicons name={rating >= star ? 'star' : rating >= star - 0.5 ? 'star-half' : 'star-outline'} size={34} color={colors.gold} /></Pressable>)}</View>
      <Text style={styles.label}>NOTES</Text><TextInput value={notes} onChangeText={setNotes} multiline style={styles.notes} placeholder="What did you notice?" placeholderTextColor="#9B958A" />
      <Text style={styles.label}>LOCATION</Text><TextInput value={location} onChangeText={setLocation} style={styles.input} placeholder="Where did you taste it?" placeholderTextColor="#9B958A" />
      <Text style={styles.label}>VISIBILITY</Text><View style={styles.visibility}>{(['public', 'followers', 'private'] as const).map((item) => <Pressable key={item} onPress={() => setVisibility(item)} style={[styles.visibilityOption, visibility === item && styles.visibilityActive]}><Text style={[styles.visibilityText, visibility === item && styles.visibilityTextActive]}>{item.charAt(0).toUpperCase() + item.slice(1)}</Text></Pressable>)}</View>
